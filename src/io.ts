...
const CommandLineInputToString = (input: CommandLineInput): string =>
  `${input}`;
...
const StringToCommandLineInput = (input: string): CommandLineInput => {
  ...
  if (input.length !== 1)
    throw new Error(`Expected string of length 1. Instead received ${input}`);
  const num_char = parseInt(input);
  const enum_length = Object.keys(CommandLineInput).length / 2;
  if (num_char < 0 || enum_length - 1 < num_char)
    throw new Error(
      `Expected number between 0 and 2 (inclusive). Instead received ${input}`
    );
  return num_char;
};

const HEADER = "   =====   VERSION INCREMENTOR   =====   \n";
...
export const GetCommandLineInput = async (): Promise<CommandLineInput> => {
  const choices: prompts.Choice[] = [
    ...
    {
      title: "cancel",
      value: CommandLineInputToString(CommandLineInput.CANCEL),
    },
  ];
