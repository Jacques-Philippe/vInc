...
export const versionNumberBeginsWithV = (version_number: string): void => {
  if (!version_number.startsWith("v"))
    throw new VersionNumberDoesntStartWithVError(version_number);
};
/**
 ...
 */
export const versionNumberMadeUpOfThreeElementsSeparatedByPeriods = (
  version_number: string
): void => {
  const elements: string[] = version_number.split(".");
  if (elements.length !== 3)
    throw new VersionNumberIsNotThreeElementsError(version_number);
  ...
  for (let index = 0; index < elements.length; index++) {
    ...
    switch (index) {
      case VersionNumbers.MAJOR: {
        if (itemIsEmpty)
          throw new VersionNumberDoesntContainMajorVersionError(version_number);
        break;
      }
      case VersionNumbers.MINOR: {
        if (itemIsEmpty)
          throw new VersionNumberDoesntContainMinorVersionError(version_number);
        break;
      }
      default: {
        if (itemIsEmpty)
          throw new VersionNumberDoesntContainPatchVersionError(version_number);
        break;
      }
    }
    number_string.split("").forEach((char) => {
      if (!charIsADigit(char))
        throw new VersionNumberIsntOnlyNumbersError(version_number);
    });
  }
};
...
const charIsADigit = (char: string): boolean => {
  if (char.length !== 1)
    throw Error(`Received ${char} expected string argument of length 1`);
